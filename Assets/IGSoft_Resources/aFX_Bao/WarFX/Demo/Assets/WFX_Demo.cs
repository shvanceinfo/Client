...
public class WFX_Demo : MonoBehaviour
{
	...
	private void selectMaterial()
	{
		...
		{
		case "Concrete":
			this.renderer.material = concrete;
			walls.transform.GetChild(0).renderer.material = concreteWall;
			walls.transform.GetChild(1).renderer.material = concreteWall;
			break;
			
		case "Wood":
			this.renderer.material = wood;
			walls.transform.GetChild(0).renderer.material = woodWall;
			walls.transform.GetChild(1).renderer.material = woodWall;
			break;

		case "Metal":
			this.renderer.material = metal;
			walls.transform.GetChild(0).renderer.material = metalWall;
			walls.transform.GetChild(1).renderer.material = metalWall;
			break;
			
		case "Checker":
			this.renderer.material = checker;
			walls.transform.GetChild(0).renderer.material = checkerWall;
			walls.transform.GetChild(1).renderer.material = checkerWall;
			break;
		}
	}
	...
	private void showHideStuff()
	{
		...
		if(ParticleExamples[exampleIndex].name.StartsWith("WFX_MF Spr"))
			m4.renderer.enabled = true;
		else
			m4.renderer.enabled = false;
		
		if(ParticleExamples[exampleIndex].name.StartsWith("WFX_MF FPS"))
			m4fps.renderer.enabled = true;
		else
			m4fps.renderer.enabled = false;
