...
{
	public static float clickInteval = 0.2f; //200ms后点击才有效
	public static float lastClickTime = 0f;  //上次点击的时间
	...
	public void ProcessTouch (bool pressed, bool unpressed)
	{
		...
		if (pressed)
		{
            if (manager.GuideManager.Instance.IsEnforceUI)
            {
                if (currentTouch.current!=null)
                if(manager.GuideManager.Instance.EnforceTagId!=currentTouch.current.GetInstanceID())
                return;
            }

			if (Time.realtimeSinceStartup - lastClickTime < clickInteval)
			{
				return;
			}
		    else
