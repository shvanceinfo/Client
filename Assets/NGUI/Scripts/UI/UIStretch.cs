...
/// <summary>
	/// 用来判断是否需要反转界面(只在halfBoth中使用)
	/// </summary>
	public bool isReverse = false;
	
	/// <summary>
	/// 需要折半处理的轴(只在halfBoth中使用)
	/// </summary>
	public HalfXY halfAxisType = HalfXY.X;
	...
	void Update ()
	{
		...
		if (style != Style.None) {
			...
			} else if (style == Style.halfBoth) {
				if (this.halfAxisType == HalfXY.X) {
					size.x = relativeSize.x * rectWidth / 2;
					 
					size.y = relativeSize.y * rectHeight;
				} else {
					size.x = relativeSize.x * rectWidth;
					size.y = relativeSize.y * rectHeight / 2;
